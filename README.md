...
|-------|---------|-------------|
| [`hpx`](https://crates.io/crates/hpx) | 1.4.0 | High Performance HTTP Client |
| [`hpx-util`](https://crates.io/crates/hpx-util) | 1.4.0 | Browser emulation profiles & Tower middleware |
| [`hpx-transport`](https://crates.io/crates/hpx-transport) | 1.4.0 | Exchange SDK toolkit (auth, WebSocket, rate limiting) |
| [`hpx-yawc`](https://crates.io/crates/hpx-yawc) | 1.4.0 | WebSocket library (RFC 6455 + compression) |
| [`hpx-fastwebsockets`](https://crates.io/crates/hpx-fastwebsockets) | 1.4.0 | Fast minimal WebSocket implementation |
...
```toml
[dependencies]
hpx = { version = "1.4.0", features = ["ws-fastwebsockets"] }
```
...
```toml
hpx = "1.4.0"  # default: boring + http1 + http2
```

**JSON API client:**

```toml
hpx = { version = "1.4.0", features = ["json", "cookies", "gzip"] }
```

**WebSocket client:**

```toml
hpx = { version = "1.4.0", features = ["ws"] }
```

**High-performance trading:**

```toml
hpx = { version = "1.4.0", features = ["simd-json", "hickory-dns", "zstd", "ws"] }
```

**Pure Rust (no C dependencies):**

```toml
hpx = { version = "1.4.0", default-features = false, features = ["rustls-tls", "http1", "http2"] }
```

**Full-featured:**

```toml
hpx = { version = "1.4.0", features = [
