...
|-------|---------|-------------|
| [`hpx`](https://crates.io/crates/hpx) | 2.4.11 | High Performance HTTP Client |
| [`hpx-emulation`](https://crates.io/crates/hpx-emulation) | 2.4.11 | Browser emulation profiles |
| [`hpx-yawc`](https://crates.io/crates/hpx-yawc) | 2.4.11 | WebSocket library (RFC 6455 + compression) |
| [`hpx-fastwebsockets`](https://crates.io/crates/hpx-fastwebsockets) | 2.4.11 | Fast minimal WebSocket implementation |
...
```toml
[dependencies]
hpx = { version = "2.4.11", features = ["ws-fastwebsockets"] }
```
...
```toml
hpx = "2.4.11"  # default: boring + http1 + http2
```

**JSON API client:**

```toml
hpx = { version = "2.4.11", features = ["json", "cookies", "gzip"] }
```

**WebSocket client:**

```toml
hpx = { version = "2.4.11", features = ["ws"] }
```

**High-performance trading:**

```toml
hpx = { version = "2.4.11", default-features = false, features = ["hft"] }
```

**Browser-like scraping / stealth profile:**

```toml
hpx = { version = "2.4.11", default-features = false, features = ["stealth"] }
```

**Pure Rust (no C dependencies):**

```toml
hpx = { version = "2.4.11", default-features = false, features = ["rustls-tls", "http1", "http2"] }
```

**Full-featured:**

```toml
hpx = { version = "2.4.11", features = [
