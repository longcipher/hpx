...
|-------|---------|-------------|
| [`hpx`](https://crates.io/crates/hpx) | 2.1.0 | High Performance HTTP Client |
| [`hpx-emulation`](https://crates.io/crates/hpx-emulation) | 2.1.0 | Browser emulation profiles |
| [`hpx-transport`](https://crates.io/crates/hpx-transport) | 2.1.0 | Exchange SDK toolkit (auth, WebSocket, rate limiting) |
| [`hpx-yawc`](https://crates.io/crates/hpx-yawc) | 2.1.0 | WebSocket library (RFC 6455 + compression) |
| [`hpx-fastwebsockets`](https://crates.io/crates/hpx-fastwebsockets) | 2.1.0 | Fast minimal WebSocket implementation |
...
```toml
[dependencies]
hpx = { version = "2.1.0", features = ["ws-fastwebsockets"] }
```
...
```toml
hpx = "2.1.0"  # default: boring + http1 + http2
```

**JSON API client:**

```toml
hpx = { version = "2.1.0", features = ["json", "cookies", "gzip"] }
```

**WebSocket client:**

```toml
hpx = { version = "2.1.0", features = ["ws"] }
```

**High-performance trading:**

```toml
hpx = { version = "2.1.0", features = ["simd-json", "hickory-dns", "zstd", "ws"] }
```

**Pure Rust (no C dependencies):**

```toml
hpx = { version = "2.1.0", default-features = false, features = ["rustls-tls", "http1", "http2"] }
```

**Full-featured:**

```toml
hpx = { version = "2.1.0", features = [
