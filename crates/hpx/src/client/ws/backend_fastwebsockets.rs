...
use super::{
    deflate::{DeflateCodec, WebSocketExtensions, negotiate_permessage_deflate},
...
impl WebSocketRequestBuilder {
    ...
    pub async fn send(self) -> Result<WebSocketResponse, Error> {
        ...
        // Set permessage-deflate extension request
...
impl WebSocketResponse {
    ...
    ///
    /// This is parsed from the `Sec-WebSocket-Extensions` response header
    ...
    pub fn extensions(&self) -> Option<WebSocketExtensions> {
        self.inner
            ...
            .and_then(|s| negotiate_permessage_deflate(s).ok())
            .flatten()
    }
...
impl WebSocket {
    ...
    pub async fn recv(&mut self) -> Option<Result<Message, Error>> {
        match self.inner.read_frame().await {
            Ok(frame) => {
                let msg = Self::decompress_frame_inner(&mut self.deflate, frame);
                Some(Ok(msg))
            }
    ...
    /// Decompress a frame if RSV1 is set.
    fn decompress_frame_inner(deflate: &mut Option<DeflateCodec>, frame: Frame) -> Message {
        if !frame.rsv1 {
        ...
        if let Some(deflate) = deflate {
            ...
            match deflate.decompress(&payload) {
                Ok(decompressed) => {
                    let mut new_frame = Frame::new(fin, opcode, None, Payload::Owned(decompressed));
                    new_frame.rsv1 = false;
                    return Message::from_frame(new_frame);
                }
                Err(_) => {
                    // Decompression failed; reconstruct the original frame
                    let mut new_frame = Frame::new(fin, opcode, None, Payload::Owned(payload));
                    new_frame.rsv1 = true;
                    return Message::from_frame(new_frame);
                }
            }
        }
        // No deflate codec; return frame as-is
    ...
    pub async fn send(&mut self, msg: Message) -> Result<(), Error> {
        let mut frame = msg.into_frame();

        // Compress if deflate is negotiated (only for text and binary frames)
        if let Some(ref mut deflate) = self.deflate {
            let is_data_frame = matches!(frame.opcode, OpCode::Text | OpCode::Binary);
            if is_data_frame && !frame.payload.is_empty() {
                let compressed = deflate
                    .compress(&frame.payload)
                    .map_err(|e| Error::upgrade(format!("deflate compression error: {e}")))?;
...
impl Stream for WebSocketRead {
    ...
    fn poll_next(mut self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Option<Self::Item>> {
        // Take the deflate codec before borrowing self.inner for the future
        let mut deflate = self.deflate.take();
        let mut send_fn = |_| async { Ok::<(), Error>(()) };

        // Scope the future so it's dropped before we write back deflate
        let result = {
        ...
        let result = match result {
            Poll::Ready(Ok(frame)) => {
                let msg = WebSocket::decompress_frame_inner(&mut deflate, frame);
                Poll::Ready(Some(Ok(msg)))
            }
            Poll::Ready(Err(e)) => Poll::Ready(Some(Err(Error::websocket(e)))),
            Poll::Pending => Poll::Pending,
        };
        // Put the deflate back
...
impl WebSocketRead {
    ...
    pub async fn recv(&mut self) -> Option<Result<Message, Error>> {
        ...
        {
            Ok(frame) => {
                let msg = WebSocket::decompress_frame_inner(&mut self.deflate, frame);
                Some(Ok(msg))
            }
            Err(e) => Some(Err(Error::websocket(e))),
        }
